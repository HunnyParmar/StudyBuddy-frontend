...
const SearchUsers = () => {
  ...
  onChange={(selectedOption) =>
    setSearchParams({
      ...searchParams,
      Country: selectedOption.value, // ✅ send country code like "IN"
    })
  }
  value={
    searchParams.Country
      ? countryOptions.find(opt => opt.value === searchParams.Country)
      : null
  }
/>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.length > 0 ? (
              users.map((user, index) => {
                const countryLabel =
                  countryOptions.find((c) => c.value === user.Country)?.label || // convert code to name
                  user.Country || // fallback to raw
                  "N/A";
                ...
                return (
                  ...
                  >
                    ...
                    <div className="px-6 py-4 text-sm text-gray-700">
                      ...
                      <div className="flex justify-between items-center mb-1">
                        <p><span className="font-medium">Country:</span> {countryLabel}</p>
                        <button
                          onClick={() => handleChatClick(user._id)}
                          className="inline-flex items-center gap-2 bg-teal-600 hover:bg-teal-800 text-white px-4 py-2 rounded-full text-sm font-medium transition cursor-pointer"
                        >
