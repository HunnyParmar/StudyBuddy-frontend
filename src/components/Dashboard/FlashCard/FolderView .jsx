...
const FolderView = ({ type = "all" }) => {
    ...
    const fetchFolders = async () => {
      try {
        ...
        const folderData = Object.keys(grouped).map(docId => ({
          ...
          topic: grouped[docId][0]?.topic || "Untitled",
          flashcardCount: grouped[docId].length,
          flashcards: grouped[docId].filter(f => f && f._id), // avoid undefined _id
        }));
  ...
  const handleDeleteFolder = async (folder) => {
    const confirm = window.confirm(
      `Are you sure you want to delete the folder "${folder.topic}" and all its flashcards?`
    );
    if (!confirm) return;
    ...
    try {
      ...
      console.log("Deleting folder:", folder.id);
        ...
        await axios.delete(`/user/flashcards/${folder.id}`, { headers });
